...
class ParkingLotForm(FlaskForm):
    prime_location_name = StringField('Location Name', validators=[DataRequired(), Length(min=3, max=100)])
    address = TextAreaField('Address', validators=[DataRequired()])
    pin_code = StringField('Pin Code', validators=[DataRequired(), Length(min=5, max=10)])
    price_per_hour = FloatField('Price per Hour (₹)', validators=[DataRequired(), NumberRange(min=1)])
